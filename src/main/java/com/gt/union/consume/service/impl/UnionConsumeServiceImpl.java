		...
		try{
			...
			for (int i=0;i<list.size();i++) {
				...
				for(int j=0;j<titles.length;j++){
					String value = "";
					if("memberName".equals(field1.getName())){//加入时间
						value = item.getMemberName();
					}
					if("items".equals(field2.getName())){//加入时间
					...
					if("cardNo".equals(field3.getName())){//加入时间
						value = item.getCardNo();
					}
					if("phone".equals(field4.getName())){//加入时间
						value = item.getPhone();
					}
					if("consumeMoney".equals(field5.getName())){//加入时间
						value = String.valueOf(CommonUtil.isEmpty(item.getConsumeMoney())? 0 : item.getConsumeMoney());
					}
					if("payMoney".equals(field6.getName())){//加入时间
						value = String.valueOf(CommonUtil.isEmpty(item.getPayMoney())? 0 : item.getPayMoney());
					}
					if("items".equals(field7.getName())){//加入时间
