...
public class ApiBaseController {
	...
	private static String VERIFYCATION = "ApiBaseController.verification";
	...
	public boolean  verification(HttpServletRequest request, HttpServletResponse response, RequestApiParam<?> requestApiParam) throws Exception {
		...
		if(StringUtil.isEmpty(signStr)){
			throw new BusinessException(VERIFYCATION, "签名为空", "签名sign不能为空");
		}
		// 解析签名
		...
		if ( SignEnum.TIME_OUT.getCode().equals( code ) ) {
			// 超过指定时间
			throw new BusinessException(VERIFYCATION, "请求超时", "请求超时");
			...
			// 签名验证错误
			throw new BusinessException(VERIFYCATION, "签名验证错误，请检查签名信息", "签名验证错误，请检查签名信息");
		} else if ( SignEnum.SERVER_ERROR.getCode().equals( code ) ) {
			logger.error(StringUtil.format("验证结果:%s", JSONObject.toJSONString(requestApiParam)));
			throw new BusinessException(VERIFYCATION, "系统错误，请检查传入参数", "系统错误，请检查传入参数");
		}
		logger.info("*******************************************签名验证结束*******************************************");
		return result;
	}
