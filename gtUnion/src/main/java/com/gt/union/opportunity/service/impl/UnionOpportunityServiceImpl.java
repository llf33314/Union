...
public class UnionOpportunityServiceImpl extends ServiceImpl<UnionOpportunityMapper, UnionOpportunity> implements IUnionOpportunityService {
    ...
    public Map<String, Object> getContactDetailByBusIdAndTgtMemberId(Integer busId, Integer tgtMemberId, Integer userMemberId) throws Exception {
        ...
        //目标盟员姓名
        result.put("tgtMemberEnterpriseName", tgtMember.getEnterpriseName());
        //(2)目标盟员所在联盟名称
        ...
        //目标盟员所在联盟名称
        result.put("tgtUnionName", unionMain.getName());
        //(3)商机推荐支付往来信息
        ...
        if (ListUtil.isNotEmpty(opportunityFromMeList)) {
            for (UnionOpportunity opportunity : opportunityFromMeList) {
                Map<String, Object> contactMap = new HashMap<>(16);
                //最后修改时间
                contactMap.put("lastModifyTime", DateUtil.getDateString(opportunity.getModifytime(), DateUtil.DATETIME_PATTERN));
                //客户姓名
                contactMap.put("clientName", opportunity.getClientName());
                //客户电话
                contactMap.put("clientPhone", opportunity.getClientPhone());
                //佣金收入：正数
                contactMap.put("brokeragePrice", opportunity.getBrokeragePrice());
                contactList.add(contactMap);
                //统计佣金往来总额
                contactMoneySum = BigDecimalUtil.add(contactMoneySum, opportunity.getBrokeragePrice());
            }
        }
        //(4-2)我接受的商机
        ...
        if (ListUtil.isNotEmpty(opportunityToMeList)) {
            for (UnionOpportunity opportunity : opportunityToMeList) {
                ...
                contactMap.put("clientPhone", opportunity.getClientPhone());
                //佣金支出：负数
                contactMap.put("brokeragePrice", BigDecimalUtil.subtract(0D, opportunity.getBrokeragePrice()).doubleValue());
                contactList.add(contactMap);
                //统计佣金往来总额
                contactMoneySum = BigDecimalUtil.subtract(contactMoneySum, opportunity.getBrokeragePrice());
            }
        }
    ...
    public Map<String, Object> getStatisticsByBusIdAndMemberId(Integer busId, Integer memberId) throws Exception {
        ...
        //收入百分比
        if (brokerageIncomeSum.doubleValue() != 0.0) {
            //未结算佣金收入占总佣金收入的百分比
            ...
            //已结算佣金收入占总佣金收入的百分比
            BigDecimal bdPaidBrokerageIncomePercent = BigDecimalUtil.subtract(1.00, bdUnPaidBrokerageIncomePercent, 4);
        ...
        if (brokerageExpenseSum.doubleValue() != 0.0D) {
            ...
            //已结算支出佣金占总支出佣金的百分比
            BigDecimal bdPaidBrokerageExpensePercent = BigDecimalUtil.subtract(1.00, bdUnPaidBrokerageExpensePercent, 4);
        ...
        //例如今天是星期四，则mondayOffSet=3
        int mondayOffset = (Calendar.getInstance().get(Calendar.DAY_OF_WEEK) - 2 + 7) % 7;
        ...
        for (int j = mondayOffset + 1; j < 7; j++) { //补充今天到周日的统计数据
            ...
            String day = DateUtil.getDateString(tempDate, DateUtil.DATE_PATTERN);
            String enDayInWeek = DateUtil.getEnDayInWeek(tempDate);
            Map<String, Double> brokerageInDayMap = new HashMap<>(16);
            brokerageInDayMap.put("paidBrokerageIncome", 0.0);
            brokerageInDayMap.put("paidBrokerageExpense", 0.0);
            Map<String, Map<String, Double>> dayMap = new HashMap<>(16);
            dayMap.put(enDayInWeek, brokerageInDayMap);
            brokerageInWeek.add(dayMap);
        }
    ...
    private Map<String, Double> getPaidBrokerageBetweenDay(Integer memberId, String strDateBegin, String strDateEnd) throws Exception {
        if (memberId != null || StringUtil.isNotEmpty(strDateBegin) || StringUtil.isNotEmpty(strDateEnd)) {
            ...
            BigDecimal incomeBrokerageSum = BigDecimal.valueOf(0.0);
    ...
    public Map<String, Object> payOpportunityQRCode(Integer busId, String ids) throws Exception {
        ...
        EntityWrapper<UnionOpportunity> wrapper = new EntityWrapper<>();
    ...
    public void insertBatchByList(List<UnionOpportunity> list, String orderNo, Integer verifierId) throws Exception {
        List<UnionBrokerageIncome> incomes = new ArrayList<>();
        List<UnionBrokeragePay> pays = new ArrayList<>();
    ...
    public Page pageFromMeMapByBusId(Page page, final Integer busId, final Integer unionId, final String isAccept
        ...
        Wrapper wrapper = new Wrapper() {
            ...
            public String getSqlSegment() {
                ...
                sbSqlSegment.append(" ORDER BY o.is_accept ASC, o.modifytime DESC, o.createtime DESC");
                return sbSqlSegment.toString();
            }
        };
        //推荐商机id
        String sqlSelect = " o.id opportunityId"
                //客户姓名
                + ", o.client_name clientName"
                //客户电话
                + ", o.client_phone clientPhone"
                //业务备注
                + ", o.business_msg businessMsg"
                //接受商机推荐的盟员名称
                + ", m.enterprise_name toEnterpriseName"
                //联盟名称
                + ", m2.name unionName"
                //受理状态
                + ", o.is_accept isAccept";
    ...
    public Page pageToMeMapByBusId(Page page, final Integer busId, final Integer unionId, final String isAccept
        ...
        String sqlSelect = " o.id opportunityId"
                ...
                + ", o.business_msg businessMsg"
                //推荐商机的盟员名称
                + ", m.enterprise_name fromEnterpriseName"
                //联盟名称
                + ", m2.name unionName"
                //受理状态
                + ", o.is_accept isAccept";
    ...
    public Page pageIncomeByBusId(Page page, final Integer busId, final Integer unionId, final Integer toMemberId
        ...
        String sqlSelect = " o.id opportunityId"
                ...
                + ", m2.name unionName"
                //受理金额
                + ", o.accept_price acceptPrice"
                //佣金金额
                + ", o.brokerage_price brokeragePrice"
                //商机推荐类型
                + ", o.type opportunityType"
                //是否已结算
                + ", CASE IFNULL(bi.opportunity_id, 0) WHEN 0 THEN " + CommonConstant.COMMON_NO
                + "  ELSE " + CommonConstant.COMMON_YES + " END isClose"
                //最后更新的时间
                + ", o.modifytime lastModifyTime";
        wrapper.setSqlSelect(sqlSelect);
        return this.selectMapsPage(page, wrapper);
    }
    ...
    @Override
    public Page pageContactByBusId(Page<UnionMember> page, Integer busId, Integer userMemberId) throws Exception {
        if (page == null || busId == null) {
            throw new ParamException(CommonConstant.PARAM_ERROR);
        }
    ...
    private List<Map<String, Object>> getContactMapList(Integer busId, Integer userMemberId, List<UnionMember> tgtMemberList) throws Exception {
        ...
        if (ListUtil.isNotEmpty(tgtMemberList)) {
            for (UnionMember tgtMember : tgtMemberList) {
                ...
                //目标盟员id
                map.put("tgtMemberId", tgtMember.getId());
                //目标盟员名称
                map.put("tgtMemberEnterpriseName", tgtMember.getEnterpriseName());
                UnionMain tgtUnion = this.unionMainService.getById(tgtMember.getUnionId());
                //目标盟员所在联盟名称
                map.put("tgtUnionName", tgtUnion != null ? tgtUnion.getName() : "");
                ...
                //往来金额
                map.put("contactMoney", contactMoney.doubleValue());
                resultList.add(map);
            }
        }
        return resultList;
    ...
    public void saveByBusIdAndMemberId(Integer busId, Integer memberId, UnionOpportunityVO vo) throws Exception {
        ...
        //推荐盟员身份id
        saveOpportunity.setFromMemberId(memberId);
        //接收盟员身份id
        saveOpportunity.setToMemberId(vo.getToMemberId());
        //是否受理
        saveOpportunity.setIsAccept(OpportunityConstant.ACCEPT_NON);
        //客户姓名
        saveOpportunity.setClientName(vo.getClientName());
        //客户电话
        saveOpportunity.setClientPhone(vo.getClientPhone());
        //业务备注
        saveOpportunity.setBusinessMsg(vo.getBusinessMsg());
        //(6)短信通知信息
        ...
        String strContent = "\"" + unionMain.getName() + "\"的盟员\""
                + fromMember.getEnterpriseName() + "\"为你推荐了客户，请到商机消息处查看。客户信息："
                + vo.getClientName() + "，" + vo.getClientPhone() + "，" + vo.getBusinessMsg();
        PhoneMessage phoneMessage = new PhoneMessage(busId, phone, strContent);
        //(7)执行保存操作
    ...
    public void updateAcceptYesByIdAndBusId(Integer opportunityId, Integer busId, Double acceptPrice) throws Exception {
        ...
        //受理状态
        updateOpportunity.setIsAccept(OpportunityConstant.ACCEPT_YES);
        //受理金额
        updateOpportunity.setAcceptPrice(acceptPrice);
        //佣金金额
        updateOpportunity.setBrokeragePrice(BigDecimalUtil.multiply(acceptPrice, BigDecimalUtil.divide(ratio.getRatio(), 100.0)).doubleValue());
        //修改时间
        updateOpportunity.setModifytime(DateUtil.getCurrentDate());
        //(7)短信通知信息
        ...
        String strContent = "\"" + unionMain.getName() + "\"的盟员\""
                + toMember.getEnterpriseName() + "\"已接受了您推荐的商机消息。客户信息："
    ...
    public void updateAcceptNoByIdAndBusId(Integer opportunityId, Integer busId) throws Exception {
        ...
        updateOpportunity.setId(opportunityId);
        //受理状态
        updateOpportunity.setIsAccept(OpportunityConstant.ACCEPT_NO);
        //修改时间
        updateOpportunity.setModifytime(DateUtil.getCurrentDate());
        //(7)短信通知信息
        UnionMain unionMain = this.unionMainService.getById(toMember.getUnionId());
        ...
        String phone = StringUtil.isNotEmpty(fromMember.getNotifyPhone()) ? fromMember.getNotifyPhone() : fromMember.getDirectorPhone();
        String strContent = "\"" + unionMain.getName() + "\"的盟员\""
                + toMember.getEnterpriseName() + "\"已拒绝了您推荐的商机消息。客户信息："
                + opportunity.getClientName() + "，"
                + opportunity.getClientPhone() + "，"
                + opportunity.getBusinessMsg();
        PhoneMessage phoneMessage = new PhoneMessage(busId, phone, strContent);
