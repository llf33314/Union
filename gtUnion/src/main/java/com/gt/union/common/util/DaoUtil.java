...
public class DaoUtil extends JdbcTemplate {
     ...
     *
     * @param dbName    数据库名称;可以不写，不写的情况下默认使用的是当前datasouce定义的数据库。
     * @param tableName 要写入的数据表名;
     * @param map       写入的数据表的数据内容;格式key=[对应操作数据表的字段名],value[对应操作数据表字段的值]
     * @return
     */
     ...
     *
     * @param dbName    数据库名
     * @param tableName 数据库中对应的数据表名;
     * @param mapList   需要存储的数据字段以及数据;
     ...
     * 批量写入一组数据到数据库
     *
     * @param dbName    数据库名称可以为null或""，表示使用当前数据源的数据库
     * @param tableName 数据表名称（必须）
     * @param mapList   添加的数据内容List集合;
     * @return 执行后影响的数据量;
     */
     ...
     * 用于生成批量写入数据库之前的sql数据;
     *
     * @param sql       传入的sql前一部分:insert into
     * @param values    传入sql的后一部分
     * @param dbName    数据库名称
     * @param tableName 数据库对应数据表名称;
     */
     ...
     *
     * @param dbName    数据库名称 可以为null或“”，默认是用DataSource配置的数据库;
     ...
     * 根据id更新一张数据表中的数据内容;
     *
     * @param dbName    数据库名,可以为null或空字符"",
     * @param tableName 数据库中对应数据表名称（必须的）
     * @param idColumn  数据表中对应的id字段名称（必须的）
     * @param idValue   数据表中对应的id字段对应的数据值（必须的）
     * @param updateMap 要更新的数据列以及对应的数据.Map Key:字段名 value:字段值
     */
