...
import com.gt.union.card.entity.UnionConsume;
import com.gt.union.card.mapper.UnionConsumeMapper;
import com.gt.union.card.service.IUnionConsumeService;
import com.gt.union.card.util.UnionConsumeCacheUtil;
...
public class UnionConsumeServiceImpl extends ServiceImpl<UnionConsumeMapper, UnionConsume> implements IUnionConsumeService {
    ...
    public List<UnionConsume> listByRootId(Integer rootId) throws Exception {
        if (rootId == null) {
            throw new ParamException(CommonConstant.PARAM_ERROR);
        }
        List<UnionConsume> result;
        // (1)cache
        String rootIdKey = UnionConsumeCacheUtil.getRootIdKey(rootId);
        ...
        entityWrapper.eq("root_id", rootId)
                .eq("del_status", CommonConstant.COMMON_NO);
        result = selectList(entityWrapper);
        setCache(result, rootId, UnionConsumeCacheUtil.TYPE_ROOT_ID);
        return result;
    }
    ...
    private void setCache(List<UnionConsume> newUnionConsumeList, Integer foreignId, int foreignIdType) {
        ...
        switch (foreignIdType) {
            ...
            case UnionConsumeCacheUtil.TYPE_CARD_ID:
                ...
                break;
            case UnionConsumeCacheUtil.TYPE_ROOT_ID:
                foreignIdKey = UnionConsumeCacheUtil.getRootIdKey(foreignId);
                break;
            default:
                break;
        }
    ...
    private void removeCache(UnionConsume unionConsume) {
        ...
        Integer rootId = unionConsume.getRootId();
        if (rootId != null) {
            String rootIdKey = UnionConsumeCacheUtil.getRootIdKey(rootId);
    ...
    private void removeCache(List<UnionConsume> unionConsumeList) {
        ...
        List<String> rootIdKeyList = getForeignIdKeyList(unionConsumeList, UnionConsumeCacheUtil.TYPE_ROOT_ID);
