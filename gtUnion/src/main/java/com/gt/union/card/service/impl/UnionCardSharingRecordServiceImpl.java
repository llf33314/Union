...
import com.gt.union.card.entity.UnionCardSharingRecord;
import com.gt.union.card.mapper.UnionCardSharingRecordMapper;
import com.gt.union.card.service.IUnionCardSharingRecordService;
import com.gt.union.card.util.UnionCardSharingRecordCacheUtil;
...
public class UnionCardSharingRecordServiceImpl extends ServiceImpl<UnionCardSharingRecordMapper, UnionCardSharingRecord> implements IUnionCardSharingRecordService {
    ...
    public List<UnionCardSharingRecord> listByRootId(Integer rootId) throws Exception {
        if (rootId == null) {
            throw new ParamException(CommonConstant.PARAM_ERROR);
        }
        List<UnionCardSharingRecord> result;
        // (1)cache
        String rootIdKey = UnionCardSharingRecordCacheUtil.getRootIdKey(rootId);
        ...
        entityWrapper.eq("root_id", rootId)
                .eq("del_status", CommonConstant.COMMON_NO);
        result = selectList(entityWrapper);
        setCache(result, rootId, UnionCardSharingRecordCacheUtil.TYPE_ROOT_ID);
        return result;
    }
    ...
    private void setCache(List<UnionCardSharingRecord> newUnionCardSharingRecordList, Integer foreignId, int foreignIdType) {
        ...
        switch (foreignIdType) {
            ...
            case UnionCardSharingRecordCacheUtil.TYPE_CARD_ID:
                ...
                break;
            case UnionCardSharingRecordCacheUtil.TYPE_ROOT_ID:
                foreignIdKey = UnionCardSharingRecordCacheUtil.getRootIdKey(foreignId);
                break;
            default:
                break;
        }
    ...
    private void removeCache(UnionCardSharingRecord unionCardSharingRecord) {
        ...
        Integer rootId = unionCardSharingRecord.getRootId();
        if (rootId != null) {
            String rootIdKey = UnionCardSharingRecordCacheUtil.getRootIdKey(rootId);
    ...
    private void removeCache(List<UnionCardSharingRecord> unionCardSharingRecordList) {
        ...
        List<String> rootIdKeyList = getForeignIdKeyList(unionCardSharingRecordList, UnionCardSharingRecordCacheUtil.TYPE_ROOT_ID);
