<template>
  <div id="itemDescription">
    <div>项目说明</div>
    <el-input type="textarea"  id="Illustration" v-model="Illustration" :rows="3" :maxlength="IllustrationMaxlength" @focus="IllustrationFocus" @blur="IllustrationBlur" @change="IllustrationKeydown($event)" @keydown="IllustrationKeydown($event)" @keyup="IllustrationKeydown($event)" @input="IllustrationKeydown($event)" @onpropertychange="IllustrationKeydown($event)" placeholder="这是项目说明">
    </el-input>
  </div>
</template>
...
export default {
  ...
  mounted: function() {
    $http
      .get(`/unionPreferentialProject/myProject/memberId/${this.unionMemberId}`)
      .then(res => {
        if (res.data.data) {
