<template>
  <div>
    <el-cascader :options="options" v-model="selectedOptions" @change="handleChange" placeholder="请选择省市区">
    </el-cascader>
  </div>
</template>
...
export default {
  ...
  watch: {
    regionOptions: function() {
      this.selectedOptions = this.regionOptions;
    }
  },
  methods: {
    handleChange(value) {
      // 拼接地区代码和名称
      let obj0 = {};
      obj0 = this.options.find((item) => {
        return item.value === this.selectedOptions[0];
      });
      this.selectedOptionsData[0] = obj0.label + ',' + this.selectedOptions[0];
      this.storeData = obj0.label;
      if (obj0.children) {
        let obj1 = {};
        obj1 = obj0.children.find((item) => {
          return item.value === this.selectedOptions[1];
        });
        this.selectedOptionsData[1] = obj1.label + ',' + this.selectedOptions[1];
        this.storeData += obj1.label;
        if (obj1.children) {
          let obj2 = {};
          obj2 = obj1.children.find((item) => {
            return item.value === this.selectedOptions[2];
          });
          this.selectedOptionsData[2] = obj2.label + ',' + this.selectedOptions[2];
          this.storeData += obj2.label;
        } else {
          this.selectedOptionsData[2] = '' + ',' + '';
        }
      } else {
        this.selectedOptionsData[1] = '' + ',' + '';
        this.selectedOptionsData[2] = '' + ',' + '';
      };
      // 向上级组件传值
      this.$emit('regionChange', this.selectedOptionsData);
      // 向store传值
      this.$store.commit('addressChange', this.storeData);
      // 触发地图更改
          ...
          "children": [
            ...
            },
            {
              ...
              "children": [
                ...
                { "value": "130428", "label": "肥乡区", },
                { "value": "130429", "label": "永年区", },
